...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(0, len(word)):
      ...
      for j in xrange(0,len(self.ALPHABET)):
        ...
        correctLetters = corruptLetters + self.ALPHABET[j]
        # The corrected word deletes character i (and lacks the start symbol)
        correction = word[1:i+1] + self.ALPHABET[j] + word[i + 1:]
        # print(correction)
  ...
  def transposeEdits(self, word):
    ...
    for index in xrange(1,len(word)):
      corruptLetters = word[index-1] + word[index];
      correctLetters = corruptLetters[::-1];
      correction = word[0:index-1] + corruptLetters[::-1] +word[index+1:];
      ret.append(Edit(correction,corruptLetters,correctLetters));
    return ret
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(0,len(word)):
      for j in xrange(0,len(self.ALPHABET)):
        ...
        corruptLetters = word[i];
        correction = word[0:i] + self.ALPHABET[j] + word[i+1:];
        # print(correction);
        # print (corruptLetters);
        # print (correctLetters);
        if (correction is not word):
          ret.append(Edit(correction,corruptLetters,correctLetters));
    return ret;
